...
export default function CTASection({ homepageContent }: CTASectionProps) {
  const title = (homepageContent as any)?.ctaTitle || 'Ready to Change a Life?'
  const description = (homepageContent as any)?.ctaDescription?.processed || ''
  const primaryLabel = (homepageContent as any)?.ctaPrimary || 'Adopt a Pet'
  const secondaryLabel = (homepageContent as any)?.ctaSecondary || 'Donate Now'
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        {description ? (
          ...
          <p className="text-primary-800 text-lg mb-8 max-w-2xl mx-auto">
            Open your heart and home to a furry friend in need. Every adoption saves two lives - the pet you adopt and the one who takes their place.
          </p>
        )}
