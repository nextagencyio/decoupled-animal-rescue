...
interface VolunteerOpportunityCardProps {
  item: DrupalVolunteerOpportunity
}

export default function VolunteerOpportunityCard({ item }: VolunteerOpportunityCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).commitment && (
            <p className="text-sm text-primary-600 font-medium mb-2">{(item as any).commitment}</p>
          )}
