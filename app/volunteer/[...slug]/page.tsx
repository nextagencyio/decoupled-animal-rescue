...
import { GET_VOLUNTEER_OPPORTUNITY_BY_PATH } from '@/lib/queries'
...
interface VolunteerOpportunityByPathData {
  route: {
    entity: DrupalVolunteerOpportunity
  } | null
}

async function getVolunteerOpportunity(path: string): Promise<DrupalVolunteerOpportunity | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<VolunteerOpportunityByPathData>({
      query: GET_VOLUNTEER_OPPORTUNITY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/volunteer/${slug.join('/')}`
  const item = await getVolunteerOpportunity(path)

  if (!item) {
    return { title: 'Volunteer Opportunity Not Found | Animal Rescue' }
  }

  return {
    title: `${item.title} | Animal Rescue`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function VolunteerOpportunityDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/volunteer/${slug.join('/')}`
  const item = await getVolunteerOpportunity(path)
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).commitment && (
                    <div>
                      <dt className="text-sm text-gray-500">Time Commitment</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).commitment}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).skillsNeeded && (
                    <div>
                      <dt className="text-sm text-gray-500">Skills Needed</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).skillsNeeded}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
