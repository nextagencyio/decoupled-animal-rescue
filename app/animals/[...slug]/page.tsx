...
import { GET_ANIMAL_BY_PATH } from '@/lib/queries'
import { DrupalAnimal } from '@/lib/types'
...
interface AnimalByPathData {
  route: {
    entity: DrupalAnimal
  } | null
}

async function getAnimal(path: string): Promise<DrupalAnimal | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<AnimalByPathData>({
      query: GET_ANIMAL_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/animals/${slug.join('/')}`
  const item = await getAnimal(path)

  if (!item) {
    return { title: 'Animal Not Found | Animal Rescue' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).breed && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).breed}</dd>
                    </div>
                  )}
                  {(item as any).age && (
                    <div>
                      <dt className="text-sm text-gray-500">Age</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).age}</dd>
                    </div>
                  )}
                  {(item as any).gender && (
                    <div>
                      <dt className="text-sm text-gray-500">Gender</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).gender}</dd>
                    </div>
                  )}
                  {(item as any).adoptionFee && (
                    <div>
                      <dt className="text-sm text-gray-500">Adoption Fee</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).adoptionFee}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
